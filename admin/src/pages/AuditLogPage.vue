...
<template>
  <AdminLayout active-menu="">
    <div class="mb-8">
      <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">审计日志</h2>
      <p class="text-gray-500 dark:text-gray-400 mt-1">管理员操作记录</p>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-4 mb-6 flex gap-4 items-center">
      <select v-model="filterAction" class="border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg px-3 py-2 text-sm" @change="page=1;loadLogs()">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm">
      <table class="w-full">
        ...
        <tbody>
          <tr v-for="(log, idx) in logs" :key="log.id" class="border-b border-gray-50 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
            <td class="p-4 text-sm text-gray-500">{{ (page - 1) * pageSize + idx + 1 }}</td>
            <td class="p-4 text-sm text-gray-800">{{ log.adminId }}</td>
            <td class="p-4"><span class="text-xs px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded-full text-gray-600">{{ log.action }}</span></td>
      ...
      <div class="p-4 flex justify-between">
        ...
        <div class="flex gap-2">
          <button v-for="p in Math.min(Math.ceil(total/pageSize), 10)" :key="p"
            ...
            :class="page===p?'bg-primary-500 text-white':'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'"
