...
export class RiskControlService {
  // 用户风险历史追踪
  private userRiskHistory: Map<number, { level: RiskLevel; count: number; lastAt: Date }[]> = new Map();
   ...
   * 连续出现中高危内容时升级风险等级
   */
  trackRisk(userId: number, level: RiskLevel): RiskLevel {
    if (!this.userRiskHistory.has(userId)) {
      this.userRiskHistory.set(userId, []);
    }

    const history = this.userRiskHistory.get(userId)!;
    history.push({ level, count: 1, lastAt: new Date() });
    ...
    // 检查是否有 3 次以上高危
    const highCount = history.filter(h => h.level === 2).length;
    if (highCount >= 3) {
      return 2;
    }

    // 检查是否有连续 5 次中危以上
    const mediumAndAbove = history.filter(h => h.level >= 1).length;
    if (mediumAndAbove >= 5) {
      return 1;
    }
   ...
   * 计算用户当前风险等级（基于整体历史）
   */
  getUserRiskLevel(userId: number): RiskLevel {
    const history = this.userRiskHistory.get(userId);
    if (!history || history.length === 0) return 0;

    const recentHistory = history.slice(-5);
    const highCount = recentHistory.filter(h => h.level === 2).length;
    const mediumCount = recentHistory.filter(h => h.level >= 1).length;
